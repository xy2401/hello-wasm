...
const playgroundIndex = read('docs/playground/index.md')
...
for (const runtime of supported) {
  const page = `docs/playground/${runtime}.md`
  expect(fs.existsSync(path.join(root, page)), `Playground 缺少物理资产页面：${runtime}`)
...
expect(viteConfig.includes("String(name).toLowerCase() === 'content-encoding'"), '本地 Vite 服务必须阻止 .gz 分片被 HTTP 层自动解压')
expect(viteConfig.includes("setHeader('Content-Type', 'application/octet-stream')"), '本地 .gz 分片必须作为二进制应用数据返回')
